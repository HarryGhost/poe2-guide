...
NAV=[('总览与需求',[('desk','整套配置 · 一屏看重点'),('attributes','完整属性需求')]),('剧情阶段',[('leveling','剧情练级 1级 → 通关')]),('异界阶段',[('start','阶段总览'),('upgrade','进阶条件与差异'),('skills','技能与精魂'),('gear','装备与独狼替代'),('tree','天赋与武器组'),('combat','实战与排障')]),('需要时查阅',[('craft','打造与涂膏'),('pitfalls','机制防坑'),('glossary','中英术语')]),('文件与依据',[('downloads','下载当前构筑'),('sources','来源与检查')])]
nav='';i=0
...
js=js.replace('/* V3_EXTENSIONS */',(P/'leveling.js').read_text(encoding='utf-8')).replace('/* V4_EXTENSIONS */',(P/'desktop.js').read_text(encoding='utf-8'))
payload=json.dumps(D,ensure_ascii=False,separators=(',',':')).replace('<','\\u003c').replace('\u2028','\\u2028').replace('\u2029','\\u2029')
page=f'''<!doctype html>
<html lang="zh-CN" data-theme="light"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><meta name="description" content="PoE2 0.5.5锐眼冰射：剧情按等级查看技能、装备、操作和任务奖励，通关后衔接异界六分支；电脑版整套概览、含装备的属性参考与可追溯来源。"><title>锐眼冰射 · 全部资料 · C雪白深栏</title><style>{css}</style></head>
<body><a class="skip" href="#main">跳到正文</a>
<aside class="sidebar" id="sidebar"><button type="button" id="menu-close" class="icon-btn menu-close">关闭目录</button><a class="brand" href="#desk"><span class="brand-mark" aria-hidden="true">↗</span><span><strong>冰射进阶</strong><small>锐眼 · 0.5.5 · 全流程资料</small></span></a><nav aria-label="章节导航">{nav}</nav><div class="side-foot">全部内容已开放<br>剧情六阶段 / 异界六分支<br>可任意提前查看；06只读<br>资料快照：2026-09-24<br><button type="button" id="reset-checks" class="btn smallbtn subtle" style="margin-top:13px;color:inherit;border-color:#46616b">清除任务勾选</button></div></aside>
<div id="nav-mask" class="mask" aria-hidden="true"></div><div id="main-wrap" class="main-wrap"><header class="topbar"><div class="actions" style="margin:0"><button type="button" class="icon-btn menu-button" id="menu-toggle" aria-expanded="false" aria-controls="sidebar">目录</button><span class="chapter-chip"><span class="longcrumb">剧情与异界指南 / </span><b id="page-name">阶段总览</b></span></div><div class="top-actions"><button class="icon-btn" id="search-open" type="button">搜索 /</button><button class="icon-btn" id="density-toggle" type="button" aria-pressed="true">紧凑</button><a class="icon-btn" href="../index.html">阶段作业首页</a></div></header>
<div class="global-bar"><div class="global-fields"><label class="level-global"><span class="field-label">当前剧情阶段</span><select id="level-stage-global" aria-label="当前剧情阶段"></select></label><label class="level-global level-input"><span class="field-label">人物等级 · 不自动切换</span><input id="level-character-level" type="number" min="1" max="100" placeholder="等级"></label><label class="current-label"><span class="field-label">当前查看BD · 全站同步</span><select id="current-bd" aria-label="当前查看BD"></select></label><label class="target-label"><span class="field-label">长期目标 · 不切换当前</span><select id="target-bd" aria-label="长期目标"><option value="0">先把当前配好</option><option value="4">04 · 保留狙击</option><option value="5">05 · 高配冰射</option></select></label><label class="mode-label"><span class="field-label">装备获取</span><select id="mode" aria-label="装备获取方式"><option value="ssf">独狼 · 自己刷</option><option value="trade">可交易 · 按缺口买</option></select></label></div><div class="global-note" id="global-note">当前与目标分开，所有章节跟随同一个当前BD。</div></div>
<main id="main" class="main" tabindex="-1"><noscript><div class="notice warn">此主界面需要JavaScript。<a href="剧情练级速查.html">剧情练级速查</a>；异界记录另有无脚本完整版本：<a href="六分支完整资料.html">打开六分支完整资料</a>。</div></noscript>{content}<footer class="footer">原始构筑：Fubgun；原站与补充映射：DSH；本版内容组织、代码与检查：GPT。<br>异界基于用户上传的0.5.5六份快照，剧情另参考作者练级页。PoE2DB为国际服简体资料，不是国服全量客户端验证。<br>全站本地运行；不使用账号、分析追踪或外部脚本。源码与记录可追溯。</footer></main></div>
...
(SITE/'数据/剧情练级结构数据.json').write_text(json.dumps(D['leveling'],ensure_ascii=False,indent=2),encoding='utf-8')
(SITE/'.nojekyll').write_text('',encoding='utf-8')
print(f'Built index.html {len(page.encode()):,} bytes; complete reference {len(full.encode()):,} bytes.')
from leveling_static import write_leveling_reference
write_leveling_reference(SITE,D,css)
