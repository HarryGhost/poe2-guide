...
const KEY='poe2-desktop-v4';let stored={};try{stored=JSON.parse(localStorage.getItem(KEY)||'{}')||{}}catch(_e){}
if(typeof stored!=='object'||Array.isArray(stored))stored={};
const validBranch=n=>Number.isInteger(Number(n))&&Number(n)>=1&&Number(n)<=6;
const state={levelStage:validBranch(stored.levelStage)?Number(stored.levelStage):3,levelTab:['skills','gear','combat','transition','tree','rewards'].includes(stored.levelTab)?stored.levelTab:'skills',density:stored.density==='comfortable'?'comfortable':'compact',levelBudgets:stored.levelBudgets&&typeof stored.levelBudgets==='object'?stored.levelBudgets:{},current:validBranch(stored.current)?Number(stored.current):1,target:[4,5].includes(Number(stored.target))?Number(stored.target):0,mode:stored.mode==='trade'?'trade':'ssf',theme:'light',checks:stored.checks&&typeof stored.checks==='object'&&!Array.isArray(stored.checks)?stored.checks:{},budgets:stored.budgets&&typeof stored.budgets==='object'&&!Array.isArray(stored.budgets)?stored.budgets:{},compareFrom:1,compareTo:2};
const ALIAS={route:'upgrade',build:'gear',trap:'pitfalls',dl:'downloads',download:'downloads',faq:'start',solo:'gear',leveling:'leveling',guide:'leveling',desk:'desk',attributes:'attributes',start:'start',upgrade:'upgrade',skills:'skills',gear:'gear',tree:'tree',combat:'combat',craft:'craft',pitfalls:'pitfalls',glossary:'glossary',downloads:'downloads',sources:'sources'};
const titles={desk:'整套配置总览',attributes:'完整属性需求',leveling:'剧情练级',start:'阶段总览',upgrade:'进阶条件与差异',skills:'技能与精魂',gear:'装备与独狼替代',tree:'天赋与武器组',combat:'实战与排障',craft:'打造与涂膏',pitfalls:'机制防坑',glossary:'中英术语',downloads:'下载当前构筑',sources:'来源与检查'};
const pages=Object.keys(titles);let activePage='desk';
function save(){try{localStorage.setItem(KEY,JSON.stringify({current:state.current,target:state.target,mode:state.mode,deskScope:deskScope,theme:state.theme,checks:state.checks,budgets:state.budgets,levelStage:state.levelStage,levelTab:state.levelTab,density:state.density,levelBudgets:state.levelBudgets}))}catch(_e){}}
...
function setTheme(){state.theme='light';document.documentElement.dataset.theme='light';document.documentElement.dataset.look='c';}
function syncGlobal(){
 ...
 syncCampaignBar();
 document.title=activePage==='leveling'?`${levelStage().label} · 剧情练级 · 锐眼冰射`:`${String(state.current).padStart(2,'0')} ${titles[activePage]} · 锐眼冰射进阶指南`;
}
...
function writeHash(anchor=activePage){const q=new URLSearchParams({bd:String(state.current)});if(activePage==='leveling'){q.set('lv',String(state.levelStage));q.set('tab',state.levelTab)}if(state.target)q.set('target',String(state.target));q.set('mode',state.mode);try{history.replaceState(null,'',`#${anchor}?${q}`)}catch(_e){}}
function renderHome(){
...
function renderSkills(){$('[data-expand-list="skill-cards"]').textContent='展开全部说明';const b=branch();$('#skills-title').textContent=`${String(b.number).padStart(2,'0')} · 技能与精魂`;$('#skills-intro').textContent=b.number===6?'完整保留原始10组记录，仅作核对；不提供可导入或可用性保证。':`${b.profile.short}。以下只展示${String(b.number).padStart(2,'0')}号的10组技能与所有内嵌连接，不混入其他分支。`;
 $('#skills-status').innerHTML=b.number===6?`<div class="notice danger slim">06只读：升华字段冲突，勿导入。${esc(b.profile.note)}</div>`:`<details class="brief-disclosure"><summary>本分支说明与原始来源（按需展开）</summary><div class="detail-body">${esc(b.profile.note)}<p class="source-note">${esc(b.sourceFile)}。列表順序保留；显示区间不等于技能等级。</p></div></details>`;
 ...
 $('#skill-cards').innerHTML=list.map(endgameRow).join('')||'<div class=empty>当前分支没有匹配。请清除关键词。</div>';
...
function renderAll(){syncGlobal();renderLeveling();renderHome();renderSkills();renderSpirit();renderGear();renderTree();renderCombat();renderCraft();renderUpgrade();renderDownloads();renderDesk();renderAttributes();}
...
function showPage(anchor='desk',focus=false){const t=document.getElementById(anchor),s=t?.closest('section')||$('#desk');activePage=s.id;$$('main>section').forEach(x=>x.hidden=x!==s);$$('.nav-link').forEach(a=>a.setAttribute('aria-current',a.hash==='#'+activePage?'page':'false'));$('#page-name').textContent=titles[activePage];syncGlobal();closeMenu();if(t&&t!==s){let p=t.parentElement;while(p&&p!==s){if(p.tagName==='DETAILS')p.open=true;p=p.parentElement}if(t.tagName==='DETAILS')t.open=true;requestAnimationFrame(()=>t.scrollIntoView({block:'start'}))}else{window.scrollTo(0,0);if(focus)$('h1',s)?.focus({preventScroll:true})}}
function parseHash(focus=false){let raw;try{raw=decodeURIComponent(location.hash.slice(1))}catch(_e){raw='start'}if(ALIAS[raw.split('?')[0]]&&ALIAS[raw.split('?')[0]]!==raw.split('?')[0]){const _q=raw.split('?')[1];raw=ALIAS[raw.split('?')[0]]+(_q?'?'+_q:'');try{history.replaceState(null,'','#'+raw)}catch(_e){}}const [anchor,query='']=raw.split('?'),q=new URLSearchParams(query);let changed=false;if(q.has('bd')&&validBranch(q.get('bd'))&&Number(q.get('bd'))!==state.current){state.current=Number(q.get('bd'));applyDefaultComparison();changed=true;['#skill-filter','#node-filter'].forEach(id=>$(id).value='');$('#skill-kind').value='all';$('#node-scope').value='all';$('#node-status').value='all'}if(q.has('target')&&[0,4,5].includes(Number(q.get('target')))){state.target=Number(q.get('target'));changed=true}if(q.has('mode')&&['ssf','trade'].includes(q.get('mode'))){state.mode=q.get('mode');changed=true}if(q.has('lv')&&validBranch(q.get('lv'))){state.levelStage=Number(q.get('lv'));changed=true}if(q.has('tab')&&['skills','gear','combat','transition','tree','rewards'].includes(q.get('tab'))){state.levelTab=q.get('tab');changed=true}if(changed){save();renderAll()}showPage(anchor||'desk',focus)}
...
document.addEventListener('change',e=>{const t=e.target;if(t.matches('[data-check]')){state.checks[t.dataset.check]=t.checked;save();resetCheckUI();updateLevelTasks()}});
$('#current-bd').addEventListener('change',e=>setCurrent(e.target.value));$('#target-bd').addEventListener('change',e=>{state.target=Number(e.target.value);save();renderHome();renderUpgrade();writeHash();notify('长期目标已更新；当前查阅BD没有改变')});$('#mode').addEventListener('change',e=>{state.mode=e.target.value;save();syncGlobal();renderHome();renderGear();renderCraft();renderLeveling();renderDesk();writeHash()});
...
$('#pitfall-list').innerHTML=detailList(pitfalls);applyDefaultComparison();setTheme();setDensity();renderAll();renderGlossary();renderSources();parseHash(false);document.body.classList.add('js-ready');
window.__guideQA={edition:DATA.edition,getState:()=>({current:state.current,target:state.target,mode:state.mode,compareFrom:state.compareFrom,compareTo:state.compareTo,page:activePage,levelStage:state.levelStage,levelTab:state.levelTab,density:state.density}),counts:{branches:DATA.branches.length,gems:Object.keys(DATA.gems).length,gearRecords:DATA.branches.reduce((n,b)=>n+b.equipment.length,0),skills:DATA.branches.reduce((n,b)=>n+b.raw.skills.length,0),nodes:DATA.branches.reduce((n,b)=>n+b.nodes.length,0)}};
})();
