...
const KEY='poe2-leveling-compact-v3';let stored={};try{stored=JSON.parse(localStorage.getItem(KEY)||'{}')||{}}catch(_e){}
if(typeof stored!=='object'||Array.isArray(stored))stored={};
const validBranch=n=>Number.isInteger(Number(n))&&Number(n)>=1&&Number(n)<=6;
const state={levelStage:validBranch(stored.levelStage)?Number(stored.levelStage):1,levelTab:['skills','gear','combat','transition','tree','rewards'].includes(stored.levelTab)?stored.levelTab:'skills',density:stored.density==='comfortable'?'comfortable':'compact',levelBudgets:stored.levelBudgets&&typeof stored.levelBudgets==='object'?stored.levelBudgets:{},current:validBranch(stored.current)?Number(stored.current):1,target:[4,5].includes(Number(stored.target))?Number(stored.target):0,mode:stored.mode==='trade'?'trade':'ssf',theme:stored.theme==='dark'?'dark':'light',checks:stored.checks&&typeof stored.checks==='object'&&!Array.isArray(stored.checks)?stored.checks:{},budgets:stored.budgets&&typeof stored.budgets==='object'&&!Array.isArray(stored.budgets)?stored.budgets:{},compareFrom:1,compareTo:2};
const ALIAS={route:'upgrade',build:'gear',trap:'pitfalls',dl:'downloads',download:'downloads',faq:'start',solo:'gear',leveling:'leveling',guide:'leveling',start:'start',upgrade:'upgrade',skills:'skills',gear:'gear',tree:'tree',combat:'combat',craft:'craft',pitfalls:'pitfalls',glossary:'glossary',downloads:'downloads',sources:'sources'};
const titles={leveling:'剧情练级',start:'阶段总览',upgrade:'进阶条件与差异',skills:'技能与精魂',gear:'装备与独狼替代',tree:'天赋与武器组',combat:'实战与排障',craft:'打造与涂膏',pitfalls:'机制防坑',glossary:'中英术语',downloads:'下载当前构筑',sources:'来源与检查'};
const pages=Object.keys(titles);let activePage='leveling';
function save(){try{localStorage.setItem(KEY,JSON.stringify({current:state.current,target:state.target,mode:state.mode,theme:state.theme,checks:state.checks,budgets:state.budgets,levelStage:state.levelStage,levelTab:state.levelTab,density:state.density,levelBudgets:state.levelBudgets}))}catch(_e){}}
...
function renderAll(){syncGlobal();renderLeveling();renderHome();renderSkills();renderSpirit();renderGear();renderTree();renderCombat();renderCraft();renderUpgrade();renderDownloads();}
...
function showPage(anchor='leveling',focus=false){const t=document.getElementById(anchor),s=t?.closest('section')||$('#leveling');activePage=s.id;$$('main>section').forEach(x=>x.hidden=x!==s);$$('.nav-link').forEach(a=>a.setAttribute('aria-current',a.hash==='#'+activePage?'page':'false'));$('#page-name').textContent=titles[activePage];syncGlobal();closeMenu();if(t&&t!==s){let p=t.parentElement;while(p&&p!==s){if(p.tagName==='DETAILS')p.open=true;p=p.parentElement}if(t.tagName==='DETAILS')t.open=true;requestAnimationFrame(()=>t.scrollIntoView({block:'start'}))}else{window.scrollTo(0,0);if(focus)$('h1',s)?.focus({preventScroll:true})}}
function parseHash(focus=false){let raw;try{raw=decodeURIComponent(location.hash.slice(1))}catch(_e){raw='start'}let _al=ALIAS[raw.split('?')[0]];if(_al&&_al!==raw.split('?')[0]){const _q=raw.split('?')[1];raw=_al+(_q?'?'+_q:'');try{history.replaceState(null,'','#'+raw)}catch(_e){}}const [anchor,query='']=raw.split('?'),q=new URLSearchParams(query);let changed=false;if(q.has('bd')&&validBranch(q.get('bd'))&&Number(q.get('bd'))!==state.current){state.current=Number(q.get('bd'));applyDefaultComparison();changed=true;['#skill-filter','#node-filter'].forEach(id=>$(id).value='');$('#skill-kind').value='all';$('#node-scope').value='all';$('#node-status').value='all'}if(q.has('target')&&[0,4,5].includes(Number(q.get('target')))){state.target=Number(q.get('target'));changed=true}if(q.has('mode')&&['ssf','trade'].includes(q.get('mode'))){state.mode=q.get('mode');changed=true}if(q.has('lv')&&validBranch(q.get('lv'))){state.levelStage=Number(q.get('lv'));changed=true}if(q.has('tab')&&['skills','gear','combat','transition','tree','rewards'].includes(q.get('tab'))){state.levelTab=q.get('tab');changed=true}if(changed){save();renderAll()}showPage(anchor||'leveling',focus)}
